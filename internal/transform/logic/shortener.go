...
func NewShortenerLogic(d *dao.DAO) *ShortenerLogic {
...
// Shorten 长链转短链核心业务逻辑
func (l *ShortenerLogic) Shorten(ctx context.Context, originalURL string) (string, error) {
	if originalURL == "" {
	...
	if err := l.dao.SaveURL(ctx, mapping); err != nil {
		return "", err
	}
...
// Expand 短码还原长链接核心业务逻辑
func (l *ShortenerLogic) Expand(ctx context.Context, shortCode string) (string, error) {
	if shortCode == "" {
		return "", errors.New("短码不能为空")
	}

	// 调 DAO 层按 Cache-Aside 策略查询（先查 Redis，未命中查 MySQL）
	return l.dao.GetOriginalURL(ctx, shortCode)
}
